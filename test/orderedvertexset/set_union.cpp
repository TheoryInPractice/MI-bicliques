...
int test_orderedvertexset_set_union(int argc, char ** argv) {
    ...
    // add nothing new
    OrderedVertexSet set_two((std::vector<size_t>){3,5});

    // add something at the start
    OrderedVertexSet set_three((std::vector<size_t>){1, 3, 5});

    // add things inbetween
    OrderedVertexSet set_four((std::vector<size_t>){5, 6, 7, 8, 9});

    // add things at the end
    OrderedVertexSet set_five((std::vector<size_t>){3, 5, 8, 10});

    // Check union with a subset
    auto new_set = set_one.set_union(set_two);
    if (!(set_one == new_set)) {
    ...
    OrderedVertexSet true_union_three((std::vector<size_t>){1, 3, 5, 7, 9});
    OrderedVertexSet computed_union_three = set_one.set_union(set_three);
    ...
    OrderedVertexSet true_union_four((std::vector<size_t>){3, 5, 6, 7, 8, 9});
    OrderedVertexSet computed_union_four = set_one.set_union(set_four);
    ...
    OrderedVertexSet true_union_five((std::vector<size_t>){3, 5, 7, 8, 9, 10});
    OrderedVertexSet computed_union_five = set_one.set_union(set_five);
