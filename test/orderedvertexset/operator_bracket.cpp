...
int test_orderedvertexset_operator_bracket(int argc, char **argv) {
    ...
    OrderedVertexSet set_one(true_vector);
    OrderedVertexSet set_two((std::vector<size_t>) {3,5,7,9,10});
    ...
    for ( size_t idx = 0; idx < true_vector.size(); idx++) {
        if (true_vector[idx] != set_one[idx]) {
            std::cout << "Entry OrderedVertexSet[" << idx << "] incorrect." << std::endl;
            error = true;
        }
    }
