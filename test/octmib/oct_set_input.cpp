...
/**
 * Test the OCTMIB.cpp algorithm passing in an OCT set as optional parameter.
 */
int test_octmib_oct_set_input(int argc, char ** argv) {
    ...
    // Set up prescribed OCT decomposition
    const OrderedVertexSet OCT_set_raw((std::vector<size_t>){2,8});
    const OrderedVertexSet left_set_raw((std::vector<size_t>){0,3,7,9,10,11,12,13});
    const OrderedVertexSet right_set_raw((std::vector<size_t>){1,4,5,6});
    ...
    for (size_t idx = 0; idx < orderings.size(); idx++) {
        ...
        // relabel the desired OCT set
        auto oct_set_input = convert_node_labels_OVS(OCT_set_raw, reverse_ordering);
        auto left_set_input = convert_node_labels_OVS(left_set_raw, reverse_ordering);
        auto right_set_input = convert_node_labels_OVS(right_set_raw, reverse_ordering);

        // Load the ground truth into a hashtable, each biclique as a string
        // mibs_map  -- to track how many times OCTMIB finds each mib
        ...
        auto mibs_computed = octmib(subgraph, oct_set_input, left_set_input, right_set_input);
