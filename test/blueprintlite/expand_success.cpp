...
/**
 * Test BlueprintLite::expand when every condition passes
 */
int test_blueprintlite_expand_success(int argc, char ** argv) {
    ...
    // Expand on vertex
    ExpansionResult expand_result = test_blueprint.expand(10,duplicates_list);

    // Should return SUCCESS
    // check return type
    if (expand_result.get_return_type() != ExpansionResult::RETURN_TYPE::SUCCESS ) {
        std::cout << "Expanding on successful node should return RETURN_TYPE::SUCCESS." << std::endl;
        error = true;
    }
