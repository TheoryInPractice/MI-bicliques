 ...
 * expanded node yields a duplicated biclique.
 */
int test_blueprintlite_expand_duplicate_check(int argc, char ** argv) {
    ...
    iterative_wait = (std::vector<size_t>) {1};

    test_blueprint.update_iterative_ptrs(std::make_shared<std::vector<size_t>>(iterative_wait),
                                         std::make_shared<OrderedVertexSet>(iterative_in),
                                         std::make_shared<std::vector<size_t>>(iterative_past));

    // Expand on vertex that yields the duplicate biclique
    ...
    if (expand_result.get_return_type() != ExpansionResult::RETURN_TYPE::DISCONTINUE ) {
        std::cout << "Expanding on node that yields duplicate should return RETURN_TYPE::DISCONTINUE." << std::endl;
        error = true;
    }
