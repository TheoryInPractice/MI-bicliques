...
/**
 * Test BlueprintLite::expand, failure condition in which
 * adding the expanded node makes completely_connect_in empty.
 */
int test_blueprintlite_expand_failure_empty_cc_in(int argc, char ** argv) {
    ...
    // Blueprint for DELAY_CONDITION = SATISFIED
    size_t biclique_blueprint_id = 0;
    std::vector<size_t> iterative_wait( {1,10} );
    OrderedVertexSet iterative_in( (std::vector<size_t>){3,7,9} );
    ...
    // After expanding on vertex
    size_t expanding_vertex = 1;
    ...
    // Expand on vertex that has no edges to completely_connected_in
    ...
    if (expand_result.get_return_type() != ExpansionResult::RETURN_TYPE::BAN ) {
        std::cout << "Expanding on node with no connection to completely_connected_in should yield RETURN_TYPE::BAN." << std::endl;
        error = true;
    }
    // check returned vertex
    size_t return_vertex = expand_result.get_vertex();
    if ( return_vertex != expanding_vertex ) {
        std::cout << "Expanding on node with no connection to completely_connected_in should return vertex " << expanding_vertex << ", not " << return_vertex << std::endl;
        error = true;
    }
