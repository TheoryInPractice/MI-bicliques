 ...
 */
int test_blueprintlite_next_future_maximal(int argc, char ** argv) {
    ...
    // Test get_last_vertex()
    if (test_blueprint.get_last_vertex() != g.get_num_vertices()) {
        std::cout << "BlueprintLite::get_last_vertex() is wrong: last vertex should be ";
        std::cout << g.get_num_vertices() << " not ";
        std::cout << test_blueprint.get_last_vertex() << std::endl;
        error = true;
    }

    // Test get_next_future_maximal()
    test_blueprint.set_next_future_maximal(g.get_num_vertices());
    if (test_blueprint.get_next_future_maximal() != g.get_num_vertices()) {
        std::cout << "BlueprintLite::get_next_future_maximal() wrong: ";
        std::cout << " should be " << g.get_num_vertices();
        std::cout << " not " << test_blueprint.get_next_future_maximal();
        std::cout << std::endl;
        error = true;
    }

    // Test get_next_future_maximal()
    test_blueprint.set_next_future_maximal(4);
    if (test_blueprint.get_next_future_maximal() != 4) {
        std::cout << "BlueprintLite::get_next_future_maximal() wrong: ";
